...
OUT = os.path.join(REPO_ROOT, "results", "aa_providers_scored.xlsx")
...
line("数据来源", f"Artificial Analysis providers leaderboard（{today_str} 抓取）：按 Model Slug 去重后取前 15% = {keep} 行。")
...
line("归一化", "Min-Max：各指标按全量样本实测 min/max 线性映射到 0–100。属『相对当前榜单』非理论满分。")
line("缺失值预测", "用『多变量岭回归 + 迭代填补』预测：以该模型的其他 8 个基准指标 + Intelligence Index 共 9 个特征联合预测目标列；多重缺失的行迭代多轮直至收敛。预测值裁剪到该列 P95。红色单元格=被预测值；(norm) 列同步标红；Imputed 列标 (reg)。")
line("成本口径", "每 1M token 消耗：70% 为输入 / 30% 为输出，且 50% 输入命中提示缓存。")
line("成本公式", "Total $/1M = 0.35×输入价 + 0.35×缓存命中价 + 0.30×输出价（USD/百万token）。缓存命中价缺失时回退输入价。")
line("解读提醒", "① 高分代表『在当前样本中靠前』，非理论能力满分。② 含红色格的行其缺失项由回归预测。③ 价格为快照，随服务商调价变动。")
R2 = {}
for m in METRICS:
    Xtr, ytr = [], []
    for i in range(len(rows)):
