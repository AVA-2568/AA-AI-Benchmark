...
面向**真实编程 / 智能体 / 复杂工作流**，全面对齐 OpenAI（GPT-6 / 5.x）与 Anthropic（Claude Fable / Opus）两家官方旗舰发布主表使用的核心共识基准，选取 7 个高难硬核指标归入 5 大类：

| 大类（权重） | 指标 | 子权重 | 全局权重 | 官方来源依据 |
|---|---|---|---|---|
| **代码与 Agent** (35%) | **Terminal-Bench 4.0** | 45% | 15.75% | 两家官方主表 Agentic Coding 第一项（真实终端操作） |
| | **DeepSWE v1.1** | 35% | 12.25% | 两家官方主表 Software Engineering（真实 GitHub Issue） |
| | **LiveBench Coding** | 20% | 7.00% | 防污染独立代码生成 |
| **业务与工作流自动化** (15%) | **AutomationBench** | 100% | 15.00% | 两家官方主表 Business Workflows |
| **指令遵循与长上下文** (20%) | **LiveBench Instruction Following** | 50% | 10.00% | 复杂指令操控 |
| | **LCR** | 50% | 10.00% | 1M 长文本检索可靠性 |
| **终极科学与推理** (20%) | **Humanity's Last Exam (HLE)** | 100% | 20.00% | 两家官方主打终极多学科博士级考场 |
| **事实准确性** (10%) | **Omniscience Index** | 100% | 10.00% | 防幻觉事实基准 |

全局权重之和 = 1.00。

**设计逻辑**：
1. **彻底淘汰学术刷题与已饱和指标**：MMLU 与 GPQA Diamond（真实值已达 0.94+ 失去区分度）全面退出通用榜，知识与深度推理交由两家共同支持的终极博士考场 **HLE**（权重由 10% 提升至 20%）；
2. **多源双重互锁的 Agent 标尺**：不再单独依赖单一代码指标，而是由 **`Terminal-Bench 4.0`**（真实命令行环境交互）与 **`DeepSWE`**（长程仓库 Issue 修复）交叉互锁，两家官方均以此拉开智能体代差；
3. **引入端到端真实业务流**：两家官方同时采用的 **`AutomationBench`** 正式入榜，填补过去只测算法题、不测复杂办公工作流自动化能力的盲区。

### 多模态（Vision）特性加分与标识

在日常代码排错（看终端截图、看 UI 设计稿）、文档阅读与日常选型中，模型**“具备图片识别能力”**还是**“纯文本模型”**具有质的体验差异。

- **不跑学术视觉跑分**：MMMU 等多模态考试题更新滞后、偏题怪题多，且无法对齐文本 token 价格；
- **实用性特性加分（Feature Bonus）**：在 `scripts/model_registry.json` 中给模型配置显式属性 `vision: true / false`。通用榜中引入特性加分项 `feature_bonuses: {"vision": 2.0}`：
  $$\text{Final Total} = \min(100.0, \text{Weighted Total} + (\text{vision} ? 2.0 : 0))$$
  在 0–100 的固定锚点体系中，2.0 分能在同等预算档位下，将具备看图能力的模型自然凸显在最优前沿线上，真实指导选型；
- **全链路标识**：CSV 与 Top 15 表格增设 `Vision` 列（`👁️` / `-`），前沿散点图区分实心圆点（支持视觉）与三角点（纯文本）。
