...
- **成本估算（真实口径 `Effective $/1M`）**：主流厂商按 **90% 缓存命中率**（`provider_cache_rates`）+ 真实缓存命中价（缺失按同厂商均值回退，**缓存写入价按 20% 写入占比混合**），再乘订阅计划折扣（`plans`：Copilot/MiniMax 官方额度制 ×0.019–0.667；ChatGPT/Claude/SuperGrok/Kimi 社区实测隐含价值 ×0.014–0.22）
...
A: 两类口径：① **官方额度制**（GitHub Copilot、MiniMax Token Plan）——月费买到确定额度（Copilot $10/$39/$100 → $15/$70/$200 API 用量；MiniMax ¥49/¥119/¥469 → 6/18/55 亿 token，且**含 API 额度**），折扣确定（Copilot ×0.50-0.67；MiniMax ×0.019-0.024）；② **社区实测隐含价值**——跑满订阅限额时按 API 牌价折算价值远超月费：SemiAnalysis（2026-06）实测 ChatGPT Plus $20 ≈ $700（35x）、Claude Pro $20 ≈ $400（20x）、ChatGPT Pro 20x $200 ≈ $14,000（70x）；xAI SuperGrok ≈5.3x（agentplans.fyi）；Kimi 会员 ≈4.5x（zhiqiang.org 实测）。榜单按 `monthly ÷ 隐含价值` 折合单价系数，多个计划命中取最低折扣。**注意**：② 类是用满额度上限（重度订阅用户）的成本下界，普通用户达不到；且行业正收窄该窗口（Anthropic 2026-06 起 agent 调用移出订阅池）。Google / 智谱 / 阶跃 / 混元 / 通义 / 豆包 / 文心 无可靠 token 折算依据或非额度制，未收录。不符你的使用强度可在 `config.json` 调整。
