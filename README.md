...
👉 [查看完整排名（Markdown 表格，`*` = 填补值）](results/ranking.md)
...
- 缺失值用**多变量岭回归**填补（仅用评分指标交叉预测，不含 Intelligence Index）；`*` = 常规填补，`**` = 低可信填补（训练样本 < 50）
...
由 GitHub Actions 驱动，**每月 1 号**自动抓取、重算排名并推送更新。[Actions](https://github.com/AVA-2568/AA-AI-Benchmark/actions) 也可手动触发。失败自动开 Issue。
...
├── config.json       # 评分权重与参数
├── scripts/          # 数据流水线
├── results/          # ranking.md（完整排名）+ validation.json
├── .github/          # CI 自动化
└── README.md
```
...
- 完整排名中 `*` 表示回归预测填补，`**` 表示低可信填补，优先参考无标注行
- 价格为抓取时快照，随服务商调价变动
