...
- **成本估算（真实口径 `Effective $/1M`）**：主流厂商按 **90% 缓存命中率**（`provider_cache_rates`）+ 真实缓存命中价（缺失按同厂商均值回退，**缓存写入价按 20% 写入占比混合**），再乘订阅计划折扣（`plans`：Copilot 官方额度 ×0.50–0.67；ChatGPT/Claude/SuperGrok 社区实测隐含价值 ×0.014–0.19）
